...
def get_flight_addresses_from_airports(departure_airport, destination_airport):
    ...
    for state in api.get_states(bbox=bbox):
...
def get_states_from_addresses(addresses):
    ...
    api = OpenSkyApi()
    return [state for state in api.get_states(icao24=addresses) if check_valid_state(state)]
...
def get_bounding_box_from_airports(departure_airport, destination_airport):
    ...
    return (
        min(departure_airport.latitude, destination_airport.latitude), 
        max(departure_airport.latitude, destination_airport.latitude),
        min(departure_airport.longitude, destination_airport.longitude), 
        max(departure_airport.longitude, destination_airport.longitude)
    )
...
def track_flights_from_airports(departure_airport, destination_airport, round_trip_mode=False):
